...
(function(global) {
  ...
  function initNeedleClient() {
    ...
    window.addEventListener('message', event => {
      // Only the hosting MTO app may report a commit result.
      if (event.source !== window.parent || window.parent === window || !event.data) return;
      if (event.data.type === 'telecom-mto-3d-cable-applied') {
        if (event.data.ok) {
          // The app sends the tag and id at the top level, not nested under `cable`.
          const label = event.data.tag || event.data.cableId || '';
          logTranscript('agent', `✔ Cable ${label} successfully integrated into 2D drawing & BOQ!`);
        } else {
